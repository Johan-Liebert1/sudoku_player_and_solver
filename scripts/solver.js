...
const check_number_validity = (board, number, row, col) => {
    return (
        check_row_validity(board, number, row) &&
        check_col_validity(board, number, col) &&
        check_box_validity(board, number, [
            Math.floor(row / 3),
            Math.floor(col / 3),
        ])
    );
...
const solve_board = (board, showSteps = false) => {
    if (!find_unfilled(board)) return true;
    else lst = find_unfilled(board);
    ...
    for (let i = 1; i < 10; i++) {
        if (
            check_row_validity(board, i, row) &&
            check_col_validity(board, i, col) &&
            check_box_validity(board, i, [
                Math.floor(row / 3),
                Math.floor(col / 3),
            ])
        ) {
            ...
            if (solve_board(board, showSteps)) return true;
